...
in {
  ...
  config =
    let
      ...
      syncConfigContents = syncCfg: optionalString (syncCfg.enable or false) ''
...
sync:
  ...
  password: "${cfg.sync.password}"
    ...
    '';
      ...
      smosConfigContents = concatStringsSep "\n" [
      ...
      backupSmosService = {
        ...
        Service = {
          ExecStart = "${pkgs.writeShellScript "backup-smos-service-ExecStart"
            ''
      ...
      syncSmosService = {
        ...
        Service = {
          ExecStart = "${pkgs.writeShellScript "sync-smos-service-ExecStart" 
            ''
