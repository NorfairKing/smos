        ...
        forAllValid $ \relFile ->
          withSystemTempDir "smos-shutdown-test" $ \tdir -> do
            ...
            withSmosInstance config (Just $ StartingFile startupFile) $ \smos1 -> do
              threadDelay 100_000 -- Wait a bit to be sure that smos definitely tried to take the lock
              ...
              withSmosInstance config (Just $ StartingFile startupFile) $ \smos2 -> do
                threadDelay 100_000 -- Wait a bit to be sure that smos definitely tried to take the lock and exited
