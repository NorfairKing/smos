      ...
      forAllValid $ \templatePath ->
        withInterestingStore $ \dc -> do
          ...
          context "yesterday schedule" $
            runEvaluatingLogT (handleScheduleItem dc historyBefore twoDaysAgoNight sn item) `shouldReturn` Just yesterdayNight
          ...
          context "today schedule" $
            runEvaluatingLogT (handleScheduleItem dc historyAfter tonight sn item) `shouldReturn` Just tonight
          ...
          context "today schedule again" $
            runEvaluatingLogT (handleScheduleItem dc historyAfter tonight sn item) `shouldReturn` Nothing

runEvaluatingLogT :: LoggingT m a -> m a
runEvaluatingLogT = flip runLoggingT evaluatingLog
