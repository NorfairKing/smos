...
spec = do
  ...
  describe "timeP" $ do
    ...
    it "parses is correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ "s") (Seconds i)
    it "parses i sec correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " sec") (Seconds i)
    it "parses i secs correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " secs") (Seconds i)
    it "parses i second correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " second") (Seconds i)
    it "parses i seconds correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " seconds") (Seconds i)
    it "parses im correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ "m") (Minutes i)
    it "parses i min correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " min") (Minutes i)
    it "parses i mins correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " mins") (Minutes i)
    it "parses i minute correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " minute") (Minutes i)
    it "parses i minutes correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " minutes") (Minutes i)
    it "parses ih correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ "h") (Hours i)
    it "parses i hr correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " hr") (Hours i)
    it "parses i hrs correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " hrs") (Hours i)
    it "parses i hour correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " hour") (Hours i)
    it "parses i hours correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " hours") (Hours i)
    it "parses id correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ "d") (Days i)
    it "parses i day correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " day") (Days i)
    it "parses i days correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " days") (Days i)
    it "parses iw correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ "w") (Weeks i)
    it "parses i wk correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " wk") (Weeks i)
    it "parses i wks correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " wks") (Weeks i)
    it "parses i week correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " week") (Weeks i)
    it "parses i weeks correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " weeks") (Weeks i)
    it "parses imo correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ "mo") (Months i)
    it "parses i mos correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " mos") (Months i)
    it "parses i month correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " month") (Months i)
    it "parses i months correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " months") (Months i)
    it "parses iy correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ "y") (Years i)
    it "parses i ys correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " ys") (Years i)
    it "parses i year correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " year") (Years i)
    it "parses i years correctly" $
      forAllValid $
        \i -> parseJust timeP (pack $ show i ++ " years") (Years i)
