...
data RRule
  = RRule
      { -- | The FREQ rule part identifies the type of recurrence rule.
        ...
        --  January 1.
        rRuleByWeekNo :: !(Maybe (NonEmpty Int)),
        -- | The BYMONTH rule part specifies a COMMA-separated list of months of the year.
        --
        -- Valid values are 1 to 12.
        rRuleByMonth :: !(Maybe (NonEmpty Word)),
        ...
        -- rule.
        rRuleBySetPos :: !(Maybe (NonEmpty Int))
      }
    ...
    mconcat
      [ genericValidate rule,
        ...
        declare "The BYYEARDAY rule part MUST NOT be specified when the FREQ rule part is set to DAILY, WEEKLY, or MONTHLY." $ case rRuleFrequency of
          ...
          _ -> True,
        decorateList
          (maybe [] NE.toList rRuleByWeekNo)
          $ \bwn -> declare "Valid values are 1 to 53 or -53 to -1." $ bwn /= 0 && bwn >= -53 && bwn <= 53,
        decorateList
          (maybe [] NE.toList rRuleByMonth)
          $ \bm -> declare "Valid values are 1 to 12" $ bm >= 1 && bm <= 12
      ]
...
rRule freq =
  RRule
    { rRuleFrequency = freq,
      ...
      rRuleBySecond = Nothing,
      rRuleByMinute = Nothing,
      rRuleByHour = Nothing,
      rRuleByDay = Nothing,
      rRuleByMonthDay = Nothing,
      rRuleByYearDay = Nothing,
      rRuleByWeekNo = Nothing,
      rRuleByMonth = Nothing,
