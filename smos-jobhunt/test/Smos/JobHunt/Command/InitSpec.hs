  ...
  describe "initSmosFile" $
    it "makes the same smos file as before" $
      goldenSmosFile "test_resources/init.smos" $
        case initSmosFile (UTCTime (fromGregorian 2023 03 20) (timeOfDayToTime (TimeOfDay 07 14 00))) "CS SYD" (Just "https://cs-syd.eu") (Just "syd@cs-syd.eu") Nothing Nothing of
  ...
  describe "smosJobHuntInit" $
    it "can initialise an application project file" $
      withSystemTempDir "smos-jobhunt-test" $ \td -> do
        ...
        pf <- runNoLoggingT $ initApplicationProject td Nothing Nothing initSettings
        mSf <- readSmosFile pf
